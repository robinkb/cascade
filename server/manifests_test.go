...
func TestStatManifests(t *testing.T) {
	t.Run("Stat existing manifest returns 200 with correct size in Content-Length header", func(t *testing.T) {
		...
		srv := server.New(&StubRegistryService{statManifest: func(repository, reference string) (*cascade.FileInfo, error) {
		...
		AssertResponseHeader(t, response.Result(), server.HeaderContentLength, strconv.Itoa(len(manifest.Bytes())))
	...
	t.Run("Stat non-existent manifest by tag returns 404 and ErrManifestUknown", func(t *testing.T) {
		srv := server.New(&StubRegistryService{getTag: func(repository, tag string) (string, error) {
...
func TestGetManifests(t *testing.T) {
	...
	t.Run("Retrieving an existing manifest returns 200", func(t *testing.T) {
		srv := server.New(&StubRegistryService{getManifest: func(repository, reference string) (*cascade.Manifest, error) {
			if repository == name && reference == digest.String() {
		...
		AssertResponseCode(t, response.Result(), http.StatusOK)
		AssertResponseHeader(t, response.Result(), server.HeaderContentType, v1.MediaTypeImageManifest)
...
func TestPutManifest(t *testing.T) {
	t.Run("Uploading a manifest by digest returns code 201", func(t *testing.T) {
		name := RandomName()
		digest, manifest := RandomManifest()
		srv := server.New(&StubRegistryService{putManifest: func(repository, reference string, content []byte) error {
	...
	t.Run("Uploading an invalid manifest returns 400 and ErrManifestInvalid", func(t *testing.T) {
		server := server.New(&StubRegistryService{putManifest: func(repository, reference string, content []byte) error {
...
func TestDeleteManifest(t *testing.T) {
	t.Run("Deleting a manifest returns code 202", func(t *testing.T) {
		name := RandomName()
		digest := RandomDigest()
		srv := server.New(&StubRegistryService{deleteManifest: func(repository, reference string) error {
			if repository == name && reference == digest.String() {
				return nil
			}
			panic(errDataNotPassedCorrectly)
	...
	t.Run("Deleting a manifest by tag returns 202", func(t *testing.T) {
		...
		srv := server.New(&StubRegistryService{deleteTag: func(repository, tag string) error {
			if repository == name && tag == wantTag {
