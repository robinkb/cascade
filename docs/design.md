...
The cluster will depend on the registry for basic functionality, like serving as a proxy for public registries, hosting manifests for GitOps engines, and hosting VM images for node installation.
Especially hosting manifests for GitOps engine is tricky.
That means that the registry must be deployed even before the GitOps engine.
Therefore, the registry must be deployed very early as part of the control plane, right after the Kubernetes API server.
...
If one of them can be orchestrated easily, then that would also solve the replication issue.
However, these projects are also built for very large scales, sometimes spanning multiple datacenters.
Because they are built to scale, they have more complex orchestration requirements, like dedicated coordinator nodes.
None seemed feasible to orchestrate during the early control plane bootstrapping.

Given all that, I decided to implement my own registry.
